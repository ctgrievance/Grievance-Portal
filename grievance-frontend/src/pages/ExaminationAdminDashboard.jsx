...
function ExaminationAdminDashboard() {
  ...
  const fetchGrievances = async () => {
    try {
      ...
      const url = `${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/grievances/department/Examination?category=${safeCategory}`;
  ...
  const handleExportSelected = (selectedData, selectedColumns) => {
    ...
    fetch(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/grievances/export-selected`, {
      ...
      .then((blob) => {
        ...
        a.href = url; a.download = `examination_grievances_${new Date().toISOString().split('T')[0]}.xlsx`;
                  ...
                  {filteredGrievances.map((g) => (
                    <tr key={g._id} onClick={() => setSelectedGrievance(g)} style={{ cursor: "pointer" }}>
                      ...
                      <td className="message-cell" style={{ maxWidth: '200px' }}>
                        ...
                        >
                          <span style={{ wordBreak: 'break-all', lineHeight: '1.2', color: "#334155", fontWeight: "500" }}>{g.message.substring(0, 40)}{g.message.length > 40 ? "..." : ""}</span>
                        </div>
                      </td>

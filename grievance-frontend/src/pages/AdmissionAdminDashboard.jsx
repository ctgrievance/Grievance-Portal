...
function AdmissionAdminDashboard() {
  ...
  const { allowStudentRecords, allowStaffRecords, allowRegisteredStudents, allowRegisteredStaff } = useDepartmentPermissions("Admission");
  ...
  const fetchGrievances = async () => {
    try {
      const safeCategory = encodeURIComponent("Admission");
      const url = `${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/grievances/department/Admission?category=${safeCategory}`;
  ...
  const handleExportSelected = (selectedData, selectedColumns) => {
    ...
    fetch(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/grievances/export-selected`, {
      ...
      .then((blob) => {
        ...
        a.href = url; a.download = `admission_grievances_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  return (
    <div className="dashboard-container">
          ...
          <div className="header-content">
            ...
            <p>
              ...
              <span className="status-badge status-assigned" style={{ marginLeft: '10px', fontSize: '0.8rem', display: 'inline-flex', alignItems: 'center', gap: '5px' }}>
                <ShieldIcon width="14" height="14" /> Admission
              </span>
            </p>
          </div>
          <ProfileHeaderButton />
          <DepartmentSwitcher currentDepartment="Admission" />
        </div>
      ...
      <nav className="navbar">
        <ul>
          <li className="admin-nav-title"><span>Admission</span></li>
          ...
          <li><SmartAssignmentNavLink department="Admission" /></li>
        </ul>
      </nav>

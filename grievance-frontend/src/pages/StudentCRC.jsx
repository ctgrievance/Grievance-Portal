...
function StudentCRC() {
  ...
  useEffect(() => {
    const fetchIssueTypes = async () => {
      try {
        const res = await fetch(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/issue-types/department/CRC%20(Placement)`);
        if (!res.ok) {
  ...
  return (
    <div className="dashboard-container">
      ...
      {/* ✅ DYNAMIC NAVBAR */}
      <StudentNavbar activeCategory="crc (placement)" />
